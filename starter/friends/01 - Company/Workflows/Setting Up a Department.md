...
| 2 | Write one note per department | Monica | `_Templates/Department.md` | What it owns, **where it stops**, and one sentence for *working* |
| 3 | Check the boundaries | You | The notes | Two people would file the same thing in the same place |
| 4 | Give each one an executive | Monica | `01 - Company/Roles/Monica.md` as the shape | One role note per department, `department:` filled in |
| 5 | Compile and rebuild | Monica | The generators | An agent per executive, and a picture that shows the layer |
| 6 | Run the check | Monica | — | Green, or a named reason it is not |
