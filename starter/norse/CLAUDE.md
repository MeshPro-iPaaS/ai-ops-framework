...
## If this session cannot run programs

Some sessions can read and write files here and nothing more. Everything in this vault still works —
it is plain text — but the generators and the check cannot run. When that is the case:

- The generated files must be updated **by hand**, in this order: the compiled agents, then the
  register, then the site. The skill `when-you-cannot-run-programs` says what goes in each.
- **The check has not run, so nothing may claim it passed.** Leave the indicator saying so. A borrowed
  green tick is exactly the failure this contract exists to prevent.
- Say so to the person, once, plainly. Do not quietly do a worse job.

One run of `python ".ops/scripts/rebuild.py" "<this folder>"` from anyone who can, and the guarantee is back.
