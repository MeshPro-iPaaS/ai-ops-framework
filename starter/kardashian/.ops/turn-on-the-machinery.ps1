...
function Get-EmbeddedPython {
    ...
    # standard library, but turning it back on costs nothing and removes a whole class of puzzle.
...
if ($failed -eq 0) {
    ...
    Write-Host "$failed vault(s) reported something untrue. Fix the note each failure names, then run this again."
}
exit $failed
